        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg overflow-hidden">
            <div className="flex items-center p-4 text-left">
                ...
                <div className="flex-shrink-0 flex items-center ml-4">
                    ...
                    >
                        {isDeletingThisItem ? <Spinner className="w-5 h-5 text-red-500" /> : <TrashIcon className="w-5 h-5" />}
                    </button>
                    <button onClick={() => setIsOpen(!isOpen)} className="p-1 ml-1 text-gray-500" aria-expanded={isOpen}>
...
export const AnalysisHistoryModal: React.FC<AnalysisHistoryModalProps> = ({ isOpen, onClose, user, onHistoryUpdate, onUserUpdate }) => {
    ...
    const handleDelete = async (idToDelete: number) => {
        ...
        try {
            ...
            const updatedHistory = user.analysisHistory.filter(item => item.id !== idToDelete);
    ...
    const handleUpgrade = async () => {
        ...
        setError(null);
        try {
            // In a real app, the checkoutUrl would be used to redirect the user to a payment page.
            await authService.initiateProUpgrade();
            // For this demo, we simulate a successful payment by directly calling the upgrade function.
            const updatedUser = await authService.upgradeToPro();
            onUserUpdate(updatedUser); // This updates the user state in App.tsx, causing a re-render
                    ...
                    {user.is_pro ? (
                        <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-2">
                           {sortedHistory.length > 0 ? (
                                sortedHistory.map(item => <HistoryItem key={item.id} item={item} onDelete={handleDelete} deletingId={deletingId} />)
                           ) : (
                        ...
                        <div className="mt-6 p-6 bg-yellow-50 border-l-4 border-yellow-400 rounded-r-lg text-center">
                            ...
                            <p className="mt-2 text-yellow-700">Never forget what was safe to eat at your favorite restaurants. Upgrade to Pro to automatically save every menu analysis to your account.</p>
                             <button onClick={handleUpgrade} disabled={!!deletingId} className="mt-4 font-bold text-white bg-yellow-500 px-6 py-2 rounded-lg hover:bg-yellow-600 disabled:bg-gray-400">
