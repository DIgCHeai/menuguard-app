...
import { AnalysisResult, ChatMessage, User, GroundingSource, AnalysisInputType, Restaurant } from './types';
...
// New type for our fetched configuration
interface AppConfig {
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const initializeApp = async () => {
            try {
                // Fetch public keys from our new secure function
                ...
                if (!response.ok) {
                    ...
                    try {
                        ...
                        // Ignore if body isn't JSON. The status code is the best we have.
                    }
                    // This creates a more informative message for the user.
                    throw new Error(`Could not load app configuration. ${serverMessage}`);
                }
                const config: AppConfig = await response.json();
                setAppConfig(config);

                // Initialize Supabase - this must be done BEFORE any other Supabase calls
                initSupabase(config.supabaseUrl, config.supabaseAnonKey);
                const supabase = getSupabaseClient(); // Get the now-initialized client
                ...
                if (!document.getElementById(scriptId)) {
                    ...
                    script.onerror = () => setLocationError("Could not load Google Maps service.");
                    document.body.appendChild(script);
                } else {
                    setIsGoogleMapsReady(true);
                }

                // Now that services are ready, set up auth listener
        ...
        try {
            ...
            if (results.length > 0) {
                 ...
                 if (currentUser?.is_pro) {
                    const updatedUser = await authService.addAnalysisToHistory(results, currentAllergies, currentPreferences, target);
    ...
    const handleSendChatMessage = async (message: string) => {
        ...
        try {
            ...
            setConversationHistory(prev => prev ? [...prev, { role: 'model', content: errorMessage }] : [{ role: 'model', content: errorMessage }]);
        } finally {
    ...
    const handleScanSuccess = (data: string | null) => {
        if (data) {
            try { new URL(data); setMenuUrl(data); setAutoAnalysisTrigger(Date.now()); } catch (_) { setError("Scanned QR code does not contain a valid URL."); }
        }
                ...
                {isInitializing ? (
                    <>
                     ...
                     <div className="text-center max-w-lg">
                        ...
                        <p className="mt-4 text-sm text-gray-500">This can happen if the application is not configured correctly on the server. Please ensure all API keys and environment variables are set up in the Netlify dashboard and try again.</p>
                    </div>
                )}
            </div>
        );
    ...
    return (
        <div className="min-h-screen bg-gray-50 text-gray-800 font-sans">
            ...
            <AuthModal isOpen={isAuthModalOpen} onClose={() => setIsAuthModalOpen(false)} onLoginSuccess={handleLoginSuccess} initialMode={authMode} setMode={setAuthMode} />
            {currentUser && isProfileSettingsOpen && <ProfileSettings isOpen={isProfileSettingsOpen} onClose={() => setIsProfileSettingsOpen(false)} user={currentUser} onUserUpdate={updateUserState} />}
            {currentUser && isHistoryModalOpen && <AnalysisHistoryModal isOpen={isHistoryModalOpen} onClose={() => setIsHistoryModalOpen(false)} user={currentUser} />}
                    ...
                    <div className="flex items-center space-x-4">
                        {currentUser ? (
                           <div ref={profileRef} className="relative">
                                     ...
                                     <div className="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 origin-top-right">
                                        <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                            <div className="px-4 py-2 border-b border-gray-200">
                                                ...
                                                <p className={`mt-1 text-xs font-semibold ${currentUser.is_pro ? 'text-yellow-600' : 'text-gray-500'}`}>{currentUser.is_pro ? 'Pro Member' : 'Standard Member'}</p>
                                            </div>
                                            <button onClick={handleManageSettings} className="w-full text-left block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Profile Settings</button>
                                            <button onClick={handleHistoryClick} disabled={!currentUser.is_pro} className="w-full text-left block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed" role="menuitem">
                                                Analysis History { !currentUser.is_pro && <span className="text-xs text-yellow-500 font-bold">(Pro)</span>}
                                            </button>
                            ...
                            <button onClick={handleLoginClick} className="flex items-center text-sm font-semibold text-green-600 hover:text-green-800 transition-colors">
            ...
            <main ref={mainContentRef} className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
                <div className="max-w-4xl mx-auto">
                    ...
                    {showInputSection && (
                        <>
                            ...
                            <div className="bg-white p-6 sm:p-8 rounded-xl shadow-md space-y-8">
                               ...
                               <hr />
                                <MenuInput 
                                    menuText={menuText} setMenuText={setMenuText} setMenuImage={setMenuImage}
                                    menuUrl={menuUrl} setMenuUrl={setMenuUrl}
                                ...
                                />
                                <div className="pt-4">
                                   <button onClick={handleAnalyze} disabled={isLoadingAnalysis} className="w-full flex items-center justify-center bg-green-600 text-white font-bold py-3 px-6 rounded-lg text-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:bg-gray-400 disabled:cursor-wait">
                                        {isLoadingAnalysis ? ( <><Spinner className="-ml-1 mr-3 h-5 w-5 text-white" />Analyzing...</> ) : ( 'Analyze Menu' )}
                                    </button>
                                </div>
                            </div>
                        </>
                    ...
                    <NearbyRestaurants isLoading={isFindingRestaurants} error={locationError} restaurants={restaurants} onSelectRestaurant={handleSelectRestaurant} />
                    ...
                    {error && (
                        <div className="mt-6 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg flex justify-between items-center" role="alert">
                           ...
                           <button onClick={() => setError(null)} className="p-1 text-red-700 hover:text-red-900" aria-label="Dismiss error"><XCircleIcon className="w-5 h-5" /></button>
                        </div>
                    )}

                    {analysisResults && (
                        <div className='mt-8 space-y-8'>
                            <ResultsDisplay results={analysisResults} analysisSummary={analysisSummary} searchGroundingSources={searchGroundingSources} menuContext={menuContextForSuggestions} currentAllergies={currentAllergies} currentPreferences={currentPreferences} analysisInputType={analysisInputType}/>
                             {conversationHistory && <ChatInterface conversation={conversationHistory.slice(1)} onSendMessage={handleSendChatMessage} isLoading={isChatLoading} />}
                        </div>
                    )}
